...
import { SortDirection } from './list/user-sortable.directive';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.fullName.toLowerCase().includes(term.toLowerCase())
        || tables.designation.toLowerCase().includes(term.toLowerCase())
        || tables.email.toLowerCase().includes(term.toLowerCase())
        || tables.contact.toLowerCase().includes(term.toLowerCase())
        || tables.createdAt.toLowerCase().includes(term.toLowerCase());
}
...
export class UserService {
     ...
     * Get Modules
     */
    public getMoudles(accountType: number) {
        return this.http.get<any>(`${environment.costplanner.host}/module/get-main-modules/?accountType=${accountType}`).pipe(map(result => {
            return result;
        }))
    }
    ...
    public allocateAccess(data) {
        return this.http.post<any>(`${environment.costplanner.host}/module/allocate-main-modules`, { data })
            .pipe(map(result => {
            return result;
        }))
    }
    ...
    public delete(id){
        return this.http.get<any>(`${environment.costplanner.host}/administrator/user/delete/?id=${id}`).pipe(map(result => {
            return result;
        }))
    }
}
