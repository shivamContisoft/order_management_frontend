...
export class SidebarComponent implements OnInit, AfterViewInit, OnChanges {
   ...
   */
  initialize(): void {
    // this.menuItems = this.menues.MENU;
   
    
    const accountType = this.authService.getAccountType();
   
    if (parseInt(accountType) == 1) {
      this.menuItems = this.menues.MENU_ADMIN
    } else if (parseInt(accountType) == 2) {
      this.menuItems = this.menues.MENU_AM
    } else if (parseInt(accountType) == 3) {
      this.menuItems = this.menues.MENU_BDM
    } else if (parseInt(accountType) == 10) {
      this.menuItems = this.menues.MENU_OPRATIONS
    } else {
      this.menuItems = this.menues.MENU_APPROVAL
    }
  }
